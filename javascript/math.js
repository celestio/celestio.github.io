function getPosition(orbit, days, origin) {
  ...
  var meanLongitude = mod2pi((orbit.meanLongitude + orbit.meanLongitudeCoef * days) * RADS);
...
function translate(orbitPath, x, y, z) {
  transformedPath = [];
  for (point = 0; point < orbitPath.length; ++point) {
    transformedPath.push([orbitPath[point][0] + x, orbitPath[point][1] + y, orbitPath[point][2] + z]);
  }
